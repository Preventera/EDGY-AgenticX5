...
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['621', '622', '623', '624', '621-624']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'infection' OR toLower(r.description) CONTAINS 'sang')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['621', '622', '623', '624', '621-624']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'stress' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
