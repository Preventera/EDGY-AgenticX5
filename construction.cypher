...
// ============================================================================
// Secteur: Construction de bâtiments (SCIAN 236)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 18,000
...
// 
// 🪜 CHUTE: 28% des lésions - Priorité CRITIQUE
// 🔧 MECANIQUE: 22% des lésions - Priorité CRITIQUE
// ⚡ ELECTRIQUE: 12% des lésions - Priorité CRITIQUE
// 🦴 ERGONOMIQUE: 18% des lésions - Priorité ELEVE
// 🔊 BRUIT: 8% des lésions - Priorité MOYEN
// 🧪 CHIMIQUE: 7% des lésions - Priorité ELEVE
// ☀️ THERMIQUE: 5% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: ASP Construction, Travail en hauteur, LOTO, SIMDUT, Premiers soins, Espace clos
// EPI CRITIQUES: casque, harnais, lunettes, gants, bottes, gilet haute visibilité
// ============================================================================
...
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
...
// ============================================================================
// 2. 🪜 RISQUES CHUTE - 28% des lésions (CRITIQUE)
// ============================================================================

// 2.1 Identifier tous les risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
...
// 2.2 Travailleurs exposés aux risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
...
// 2.3 Zones critiques pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'glissade')
...
// 2.4 Gap certifications pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'glissade')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'ASP Construction' IN p.certifications_sst OR NOT 'Travail en hauteur' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'ASP Construction, Travail en hauteur' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 3. 🔧 RISQUES MECANIQUE - 22% des lésions (CRITIQUE)
// ============================================================================

// 3.1 Identifier tous les risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'machine')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 3.2 Travailleurs exposés aux risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'machine')
...
// 3.3 Zones critiques pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'machine')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'machine')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'LOTO' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'LOTO' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. ⚡ RISQUES ELECTRIQUE - 12% des lésions (CRITIQUE)
// ============================================================================

// 4.1 Identifier tous les risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'arc flash' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 4.2 Travailleurs exposés aux risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'arc flash' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur')
...
// 4.3 Zones critiques pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'arc flash' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'arc flash' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur')
...
// 5.2 Travailleurs exposés aux risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'posture' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'manutention')
...
// 5.3 Zones critiques pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'posture' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'manutention')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'posture' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'manutention')
...
// ============================================================================
// 6. 🔊 RISQUES BRUIT - 8% des lésions (MOYEN)
// ============================================================================

// 6.1 Identifier tous les risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'décibel' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'surdité')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 6.2 Travailleurs exposés aux risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'décibel' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'surdité')
...
// 6.3 Zones critiques pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'décibel' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'surdité')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'décibel' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'surdité')
...
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'ASP Construction, Travail en hauteur, LOTO' AS certifications_recommandees
LIMIT 30;
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Code de sécurité construction, ASP Construction, CNESST
// ============================================================================
...
// Vérification certifications requises secteur: ASP Construction, Travail en hauteur, LOTO, SIMDUT, Premiers soins, Espace clos
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['ASP Construction', 'Travail en hauteur', 'LOTO', 'SIMDUT', 'Premiers soins', 'Espace clos']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'ASP Construction, Travail en hauteur, LOTO' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['236', '237', '238', '236-238']
  ...
  AND r.categorie IN ['chute', 'mecanique', 'electrique']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'chute' THEN 1 ELSE 0 END) AS risques_cat1,
     sum(CASE WHEN r.categorie = 'mecanique' THEN 1 ELSE 0 END) AS risques_cat2,
     sum(CASE WHEN r.probabilite * r.gravite >= 15 THEN 1 ELSE 0 END) AS risques_TZ
RETURN o.name AS organisation,
       o.nb_employes AS employes,
       nb_zones, nb_risques, nb_equipes, nb_personnes,
       round(score_moyen * 100) / 100 AS score_moyen,
       risques_cat1 AS 'chute',
       risques_cat2 AS 'mecanique',
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 236 - CONSTRUCTION
