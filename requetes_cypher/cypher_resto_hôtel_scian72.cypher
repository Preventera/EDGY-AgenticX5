...
// ============================================================================
// Secteur: Hébergement et restauration (SCIAN 72)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 15,000
...
// 
// 🦴 ERGONOMIQUE: 28% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 22% des lésions - Priorité ELEVE
// 🔥 THERMIQUE: 20% des lésions - Priorité ELEVE
// 🔪 COUPURE: 15% des lésions - Priorité ELEVE
// 🧪 CHIMIQUE: 8% des lésions - Priorité MOYEN
// 🧠 PSYCHOSOCIAL: 7% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: SIMDUT, Hygiène alimentaire, Premiers soins, MAPAQ
// EPI CRITIQUES: chaussures antidérapantes, tablier, gants cuisine, gants four
// ============================================================================
...
// ============================================================================
// 2. 🦴 RISQUES ERGONOMIQUE - 28% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. 🪜 RISQUES CHUTE - 22% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 4. 🔥 RISQUES THERMIQUE - 20% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['72']
...
// 4.2 Travailleurs exposés aux risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
...
// 4.3 Zones critiques pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'SIMDUT' IN p.certifications_sst OR NOT 'Hygiène alimentaire' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, Hygiène alimentaire, Premiers soins' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 5. 🔪 RISQUES COUPURE - 15% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques coupure
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'coupure' OR toLower(r.description) CONTAINS 'lacération' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'lame' OR toLower(r.description) CONTAINS 'tranchant' OR toLower(r.description) CONTAINS 'couteau')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 5.2 Travailleurs exposés aux risques coupure
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'coupure' OR toLower(r.description) CONTAINS 'lacération' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'lame' OR toLower(r.description) CONTAINS 'tranchant' OR toLower(r.description) CONTAINS 'couteau')
...
// 5.3 Zones critiques pour risques coupure
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'coupure' OR toLower(r.description) CONTAINS 'lacération' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'lame' OR toLower(r.description) CONTAINS 'tranchant' OR toLower(r.description) CONTAINS 'couteau')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques coupure
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['72']
  AND (r.categorie = 'coupure' OR toLower(r.description) CONTAINS 'lacération' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'lame' OR toLower(r.description) CONTAINS 'tranchant' OR toLower(r.description) CONTAINS 'couteau')
...
// ============================================================================
// 6. 🧪 RISQUES CHIMIQUE - 8% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, MAPAQ, Normes travail
// ============================================================================
...
// Vérification certifications requises secteur: SIMDUT, Hygiène alimentaire, Premiers soins, MAPAQ
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['72']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'Hygiène alimentaire', 'Premiers soins', 'MAPAQ']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, Hygiène alimentaire, Premiers soins' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['72']
  ...
  AND r.categorie IN ['ergonomique', 'chute', 'thermique']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 72 - RESTO/HÔTEL
