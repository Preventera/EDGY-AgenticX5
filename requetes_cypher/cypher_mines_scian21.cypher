...
// ============================================================================
// Secteur: Extraction minière (SCIAN 21)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 3,000
...
// 
// 🔧 MECANIQUE: 28% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 18% des lésions - Priorité CRITIQUE
// ⛏️ EFFONDREMENT: 15% des lésions - Priorité CRITIQUE
// 🧪 CHIMIQUE: 12% des lésions - Priorité ELEVE
// 🔊 BRUIT: 12% des lésions - Priorité ELEVE
// 🦴 ERGONOMIQUE: 10% des lésions - Priorité ELEVE
// ⚡ ELECTRIQUE: 5% des lésions - Priorité ELEVE
//
// CERTIFICATIONS REQUISES: Module minier, Espace clos, SIMDUT, Premiers soins, Sauvetage minier
// EPI CRITIQUES: casque lampe, bottes cap acier, lunettes, protecteurs auditifs, auto-sauveteur
// ============================================================================
...
// ============================================================================
// 2. 🔧 RISQUES MECANIQUE - 28% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. 🪜 RISQUES CHUTE - 18% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 4. ⛏️ RISQUES EFFONDREMENT - 15% des lésions (CRITIQUE)
// ============================================================================

// 4.1 Identifier tous les risques effondrement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['21']
  AND (r.categorie = 'effondrement' OR toLower(r.description) CONTAINS 'effondrement')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 4.2 Travailleurs exposés aux risques effondrement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['21']
  AND (r.categorie = 'effondrement' OR toLower(r.description) CONTAINS 'effondrement')
...
// 4.3 Zones critiques pour risques effondrement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['21']
  AND (r.categorie = 'effondrement' OR toLower(r.description) CONTAINS 'effondrement')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques effondrement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['21']
  AND (r.categorie = 'effondrement' OR toLower(r.description) CONTAINS 'effondrement')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Module minier' IN p.certifications_sst OR NOT 'Espace clos' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Module minier, Espace clos, SIMDUT' AS certifications_recommandees
LIMIT 30;
...
// ============================================================================
// 6. 🔊 RISQUES BRUIT - 12% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - Loi sur les mines, RSST, CNESST secteur minier
// ============================================================================
...
// Vérification certifications requises secteur: Module minier, Espace clos, SIMDUT, Premiers soins, Sauvetage minier
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['21']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Module minier', 'Espace clos', 'SIMDUT', 'Premiers soins', 'Sauvetage minier']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Module minier, Espace clos, SIMDUT' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['21']
  ...
  AND r.categorie IN ['mecanique', 'chute', 'effondrement']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 21 - MINES
