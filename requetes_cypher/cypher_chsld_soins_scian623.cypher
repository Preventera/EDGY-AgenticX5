...
// ============================================================================
// Secteur: Établissements de soins infirmiers (SCIAN 623)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 10,000
...
// 🦠 BIOLOGIQUE: 15% des lésions - Priorité ELEVE
// ⚠️ VIOLENCE: 12% des lésions - Priorité ELEVE
// 🪜 CHUTE: 8% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: PDSB, SIMDUT, RCR, Prévention infections, Gestion comportements
// EPI CRITIQUES: gants, masque, blouse, chaussures antidérapantes
// ============================================================================
...
// ============================================================================
// 3. 🧠 RISQUES PSYCHOSOCIAL - 20% des lésions (ELEVE)
// ============================================================================
...
WHERE o.sector_scian IN ['623']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'PDSB' IN p.certifications_sst OR NOT 'SIMDUT' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'PDSB, SIMDUT, RCR' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. 🦠 RISQUES BIOLOGIQUE - 15% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['623']
...
// 4.2 Travailleurs exposés aux risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['623']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// 4.3 Zones critiques pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['623']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['623']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// ============================================================================
// 6. 🪜 RISQUES CHUTE - 8% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, MSSS, Loi sur les CHSLD
// ============================================================================
...
// Vérification certifications requises secteur: PDSB, SIMDUT, RCR, Prévention infections, Gestion comportements
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['623']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['PDSB', 'SIMDUT', 'RCR', 'Prévention infections', 'Gestion comportements']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'PDSB, SIMDUT, RCR' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['623']
  ...
  AND r.categorie IN ['ergonomique', 'psychosocial', 'biologique']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 623 - CHSLD/SOINS
