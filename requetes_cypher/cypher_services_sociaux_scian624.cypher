...
// ============================================================================
// Secteur: Services sociaux (SCIAN 624)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 4,500
...
// 
// 🧠 PSYCHOSOCIAL: 35% des lésions - Priorité CRITIQUE
// ⚠️ VIOLENCE: 25% des lésions - Priorité CRITIQUE
// 🦴 ERGONOMIQUE: 20% des lésions - Priorité ELEVE
// 🦠 BIOLOGIQUE: 10% des lésions - Priorité MOYEN
// 🪜 CHUTE: 10% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Intervention de crise, Premiers soins, RCR, SIMDUT
// EPI CRITIQUES: alarme personnelle, chaussures sécuritaires
// ============================================================================
...
// ============================================================================
// 2. 🧠 RISQUES PSYCHOSOCIAL - 35% des lésions (CRITIQUE)
// ============================================================================

// 2.1 Identifier tous les risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
...
// 2.2 Travailleurs exposés aux risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
...
// 2.3 Zones critiques pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
...
// 2.4 Gap certifications pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Intervention de crise' IN p.certifications_sst OR NOT 'Premiers soins' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Intervention de crise, Premiers soins, RCR' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 3. ⚠️ RISQUES VIOLENCE - 25% des lésions (CRITIQUE)
// ============================================================================

// 3.1 Identifier tous les risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
...
// 3.2 Travailleurs exposés aux risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// 3.3 Zones critiques pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// ============================================================================
// 4. 🦴 RISQUES ERGONOMIQUE - 20% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
...
// 4.2 Travailleurs exposés aux risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// 4.3 Zones critiques pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// ============================================================================
// 5. 🦠 RISQUES BIOLOGIQUE - 10% des lésions (MOYEN)
// ============================================================================

// 5.1 Identifier tous les risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
...
// 5.2 Travailleurs exposés aux risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// 5.3 Zones critiques pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['624']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Loi sur les services sociaux
// ============================================================================
...
// Vérification certifications requises secteur: Intervention de crise, Premiers soins, RCR, SIMDUT
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['624']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Intervention de crise', 'Premiers soins', 'RCR', 'SIMDUT']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Intervention de crise, Premiers soins, RCR' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['624']
  ...
  AND r.categorie IN ['psychosocial', 'violence', 'ergonomique']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'psychosocial' THEN 1 ELSE 0 END) AS risques_cat1,
     sum(CASE WHEN r.categorie = 'violence' THEN 1 ELSE 0 END) AS risques_cat2,
     sum(CASE WHEN r.probabilite * r.gravite >= 15 THEN 1 ELSE 0 END) AS risques_TZ
RETURN o.name AS organisation,
       o.nb_employes AS employes,
       nb_zones, nb_risques, nb_equipes, nb_personnes,
       round(score_moyen * 100) / 100 AS score_moyen,
       risques_cat1 AS 'psychosocial',
       risques_cat2 AS 'violence',
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 624 - SERVICES SOCIAUX
