...
// ============================================================================
// Secteur: Agriculture, foresterie, pêche (SCIAN 11)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 5,000
...
// ☀️ THERMIQUE: 8% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Pesticides, SIMDUT, Tracteur, Premiers soins, Tronçonneuse
// EPI CRITIQUES: bottes, gants, combinaison, masque respiratoire, lunettes
// ============================================================================
...
// ============================================================================
// 6. 🦠 RISQUES BIOLOGIQUE - 10% des lésions (MOYEN)
// ============================================================================

// 6.1 Identifier tous les risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['11']
...
// 6.2 Travailleurs exposés aux risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['11']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// 6.3 Zones critiques pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['11']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques biologique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['11']
  AND (r.categorie = 'biologique' OR toLower(r.description) CONTAINS 'bactérie' OR toLower(r.description) CONTAINS 'sang' OR toLower(r.description) CONTAINS 'piqûre' OR toLower(r.description) CONTAINS 'moisissure' OR toLower(r.description) CONTAINS 'virus' OR toLower(r.description) CONTAINS 'infection')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, UPA, MAPAQ, Pesticides
// ============================================================================
...
// Vérification certifications requises secteur: Pesticides, SIMDUT, Tracteur, Premiers soins, Tronçonneuse
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['11']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Pesticides', 'SIMDUT', 'Tracteur', 'Premiers soins', 'Tronçonneuse']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Pesticides, SIMDUT, Tracteur' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['11']
  ...
  AND r.categorie IN ['mecanique', 'ergonomique', 'chute']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 11 - AGRICULTURE
