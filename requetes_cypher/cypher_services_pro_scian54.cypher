...
// ============================================================================
// Secteur: Services professionnels et techniques (SCIAN 54)
...
// 
// 🦴 ERGONOMIQUE: 45% des lésions - Priorité CRITIQUE
// 🧠 PSYCHOSOCIAL: 30% des lésions - Priorité ELEVE
// 🪜 CHUTE: 10% des lésions - Priorité MOYEN
// ⚡ ELECTRIQUE: 8% des lésions - Priorité MOYEN
// ⚠️ VIOLENCE: 7% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Premiers soins, Ergonomie bureau
// EPI CRITIQUES: chaise ergonomique, écran ajustable, clavier ergonomique
// ============================================================================
...
// ============================================================================
// 2. 🦴 RISQUES ERGONOMIQUE - 45% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. 🧠 RISQUES PSYCHOSOCIAL - 30% des lésions (ELEVE)
// ============================================================================

// 3.1 Identifier tous les risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['54']
...
// 3.2 Travailleurs exposés aux risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
...
// 3.3 Zones critiques pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Premiers soins' IN p.certifications_sst OR NOT 'Ergonomie bureau' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Premiers soins, Ergonomie bureau' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. 🪜 RISQUES CHUTE - 10% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// 5. ⚡ RISQUES ELECTRIQUE - 8% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// 6. ⚠️ RISQUES VIOLENCE - 7% des lésions (MOYEN)
// ============================================================================

// 6.1 Identifier tous les risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['54']
...
// 6.2 Travailleurs exposés aux risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// 6.3 Zones critiques pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['54']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Normes ergonomie ANSI
// ============================================================================
...
// Vérification certifications requises secteur: Premiers soins, Ergonomie bureau
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['54']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Premiers soins', 'Ergonomie bureau']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Premiers soins, Ergonomie bureau' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['54']
  ...
  AND r.categorie IN ['ergonomique', 'psychosocial', 'chute']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'psychosocial' THEN 1 ELSE 0 END) AS risques_cat2,
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 54 - SERVICES PRO
