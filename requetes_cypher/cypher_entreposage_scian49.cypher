...
// 🪜 CHUTE: 20% des lésions - Priorité ELEVE
// 🚛 COLLISION: 10% des lésions - Priorité ELEVE
// ❄️ THERMIQUE: 10% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Cariste, SIMDUT, Rayonnage, Premiers soins
// EPI CRITIQUES: chaussures sécurité, gilet haute visibilité, gants, vêtements chauds
// ============================================================================
...
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
...
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Cariste' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Cariste' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. 🪜 RISQUES CHUTE - 20% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 5. 🚛 RISQUES COLLISION - 10% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques collision
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
  AND (r.categorie = 'collision' OR toLower(r.description) CONTAINS 'véhicule' OR toLower(r.description) CONTAINS 'renversement' OR toLower(r.description) CONTAINS 'circulation' OR toLower(r.description) CONTAINS 'collision')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 5.2 Travailleurs exposés aux risques collision
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
  AND (r.categorie = 'collision' OR toLower(r.description) CONTAINS 'véhicule' OR toLower(r.description) CONTAINS 'renversement' OR toLower(r.description) CONTAINS 'circulation' OR toLower(r.description) CONTAINS 'collision')
...
// 5.3 Zones critiques pour risques collision
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
  AND (r.categorie = 'collision' OR toLower(r.description) CONTAINS 'véhicule' OR toLower(r.description) CONTAINS 'renversement' OR toLower(r.description) CONTAINS 'circulation' OR toLower(r.description) CONTAINS 'collision')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques collision
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
  AND (r.categorie = 'collision' OR toLower(r.description) CONTAINS 'véhicule' OR toLower(r.description) CONTAINS 'renversement' OR toLower(r.description) CONTAINS 'circulation' OR toLower(r.description) CONTAINS 'collision')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Cariste' IN p.certifications_sst OR NOT 'SIMDUT' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Cariste, SIMDUT, Rayonnage' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. ❄️ RISQUES THERMIQUE - 10% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Normes rayonnage
// ============================================================================
...
// Vérification certifications requises secteur: Cariste, SIMDUT, Rayonnage, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['49', '491', '492', '493', '48-49', '484-493']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Cariste', 'SIMDUT', 'Rayonnage', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Cariste, SIMDUT, Rayonnage' AS certifications_prioritaires
LIMIT 50;
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 49 - ENTREPOSAGE
