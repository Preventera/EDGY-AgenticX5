...
// ============================================================================
// Secteur: Fabrication d'aliments (SCIAN 31)
...
// 
// 🦴 ERGONOMIQUE: 30% des lésions - Priorité CRITIQUE
// 🔧 MECANIQUE: 25% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 15% des lésions - Priorité ELEVE
// 🔥 THERMIQUE: 12% des lésions - Priorité ELEVE
// 🧪 CHIMIQUE: 10% des lésions - Priorité MOYEN
// 🔊 BRUIT: 8% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: SIMDUT, LOTO, Hygiène alimentaire, Cariste, Premiers soins
// EPI CRITIQUES: gants, tablier, bottes antidérapantes, résille, protecteurs auditifs
// ============================================================================
...
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
...
// ============================================================================
// 2. 🦴 RISQUES ERGONOMIQUE - 30% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. 🔧 RISQUES MECANIQUE - 25% des lésions (CRITIQUE)
// ============================================================================
...
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'LOTO, Cariste' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. 🪜 RISQUES CHUTE - 15% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
...
// 4.2 Travailleurs exposés aux risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
...
// 4.3 Zones critiques pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
...
// ============================================================================
// 5. 🔥 RISQUES THERMIQUE - 12% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
...
// 5.2 Travailleurs exposés aux risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
...
// 5.3 Zones critiques pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
...
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, LOTO, Hygiène alimentaire' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. 🧪 RISQUES CHIMIQUE - 10% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, MAPAQ, ACIA
// ============================================================================
...
// Vérification certifications requises secteur: SIMDUT, LOTO, Hygiène alimentaire, Cariste, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'LOTO', 'Hygiène alimentaire', 'Cariste', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, LOTO, Hygiène alimentaire' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['31', '311', '312', '31-33']
  ...
  AND r.categorie IN ['ergonomique', 'mecanique', 'chute']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 31 - AGROALIMENTAIRE
