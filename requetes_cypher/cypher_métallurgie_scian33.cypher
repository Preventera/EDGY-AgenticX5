...
// ============================================================================
// Secteur: Fabrication de machines et métaux (SCIAN 33)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 13,000
...
// 
// 🔧 MECANIQUE: 35% des lésions - Priorité CRITIQUE
// 🦴 ERGONOMIQUE: 20% des lésions - Priorité ELEVE
// 🔊 BRUIT: 15% des lésions - Priorité ELEVE
// 🧪 CHIMIQUE: 12% des lésions - Priorité ELEVE
// 🔥 THERMIQUE: 10% des lésions - Priorité ELEVE
// ⚡ ELECTRIQUE: 8% des lésions - Priorité ELEVE
//
// CERTIFICATIONS REQUISES: SIMDUT, LOTO, Soudage, Pont roulant, Cariste, Meulage
// EPI CRITIQUES: lunettes, masque soudeur, gants, tablier cuir, protecteurs auditifs
// ============================================================================
...
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
...
// ============================================================================
// 2. 🔧 RISQUES MECANIQUE - 35% des lésions (CRITIQUE)
// ============================================================================
...
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  ...
  AND (p.certifications_sst IS NULL OR NOT 'LOTO' IN p.certifications_sst OR NOT 'Pont roulant' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'LOTO, Pont roulant, Cariste' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 3. 🦴 RISQUES ERGONOMIQUE - 20% des lésions (ELEVE)
// ============================================================================

// 3.1 Identifier tous les risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
...
// 3.2 Travailleurs exposés aux risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// 3.3 Zones critiques pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// ============================================================================
// 4. 🔊 RISQUES BRUIT - 15% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
...
// 4.2 Travailleurs exposés aux risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
...
// 4.3 Zones critiques pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'SIMDUT' IN p.certifications_sst OR NOT 'LOTO' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, LOTO, Soudage' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 5. 🧪 RISQUES CHIMIQUE - 12% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 6. 🔥 RISQUES THERMIQUE - 10% des lésions (ELEVE)
// ============================================================================

// 6.1 Identifier tous les risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 6.2 Travailleurs exposés aux risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
...
// 6.3 Zones critiques pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques thermique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  AND (r.categorie = 'thermique' OR toLower(r.description) CONTAINS 'chaleur' OR toLower(r.description) CONTAINS 'brûlure' OR toLower(r.description) CONTAINS 'froid' OR toLower(r.description) CONTAINS 'four' OR toLower(r.description) CONTAINS 'congélateur' OR toLower(r.description) CONTAINS 'température')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, CSA W117.2, Code électrique
// ============================================================================
...
// Vérification certifications requises secteur: SIMDUT, LOTO, Soudage, Pont roulant, Cariste, Meulage
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'LOTO', 'Soudage', 'Pont roulant', 'Cariste', 'Meulage']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, LOTO, Soudage' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['33', '331', '332', '333', '334', '335', '336', '337', '339', '31-33']
  ...
  AND r.categorie IN ['mecanique', 'ergonomique', 'bruit']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'ergonomique' THEN 1 ELSE 0 END) AS risques_cat2,
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 33 - MÉTALLURGIE
