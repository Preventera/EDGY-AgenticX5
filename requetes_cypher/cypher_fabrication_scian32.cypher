...
// ============================================================================
// Secteur: Fabrication de produits (SCIAN 32)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 20,000
...
// 🦴 ERGONOMIQUE: 25% des lésions - Priorité CRITIQUE
// 🧪 CHIMIQUE: 15% des lésions - Priorité ELEVE
// 🔊 BRUIT: 12% des lésions - Priorité ELEVE
// 🪜 CHUTE: 10% des lésions - Priorité MOYEN
// ⚡ ELECTRIQUE: 8% des lésions - Priorité ELEVE
//
// CERTIFICATIONS REQUISES: SIMDUT, LOTO, Cariste, Pont roulant, Premiers soins
// EPI CRITIQUES: lunettes, gants, protecteurs auditifs, chaussures sécurité, casque
// ============================================================================
...
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
...
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  ...
  AND (p.certifications_sst IS NULL OR NOT 'LOTO' IN p.certifications_sst OR NOT 'Cariste' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'LOTO, Cariste, Pont roulant' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 3. 🦴 RISQUES ERGONOMIQUE - 25% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 4. 🧪 RISQUES CHIMIQUE - 15% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
...
// 4.2 Travailleurs exposés aux risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
...
// 4.3 Zones critiques pour risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
...
// ============================================================================
// 5. 🔊 RISQUES BRUIT - 12% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
...
// 5.2 Travailleurs exposés aux risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
...
// 5.3 Zones critiques pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques bruit
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  AND (r.categorie = 'bruit' OR toLower(r.description) CONTAINS 'surdité' OR toLower(r.description) CONTAINS 'bruit' OR toLower(r.description) CONTAINS 'audition' OR toLower(r.description) CONTAINS 'décibel')
...
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, LOTO, Cariste' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. 🪜 RISQUES CHUTE - 10% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Code électrique, Normes machines
// ============================================================================
...
// Vérification certifications requises secteur: SIMDUT, LOTO, Cariste, Pont roulant, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'LOTO', 'Cariste', 'Pont roulant', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, LOTO, Cariste' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['32', '321', '322', '323', '324', '325', '326', '327', '31-33']
  ...
  AND r.categorie IN ['mecanique', 'ergonomique', 'chimique']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 32 - FABRICATION
