...
// ============================================================================
// Secteur: Entrepreneurs spécialisés (SCIAN 238)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 12,000
...
// 
// 🪜 CHUTE: 25% des lésions - Priorité CRITIQUE
// ⚡ ELECTRIQUE: 20% des lésions - Priorité CRITIQUE
// 🔧 MECANIQUE: 18% des lésions - Priorité ELEVE
// 🦴 ERGONOMIQUE: 15% des lésions - Priorité ELEVE
// 🧪 CHIMIQUE: 12% des lésions - Priorité ELEVE
// 🔥 THERMIQUE: 10% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: ASP Construction, Travail en hauteur, LOTO, Électricité, Soudage, SIMDUT
// EPI CRITIQUES: casque, harnais, lunettes, gants isolants, masque soudeur
// ============================================================================
...
// ============================================================================
// 2. 🪜 RISQUES CHUTE - 25% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. ⚡ RISQUES ELECTRIQUE - 20% des lésions (CRITIQUE)
// ============================================================================

// 3.1 Identifier tous les risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['238']
...
// 3.2 Travailleurs exposés aux risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
...
// 3.3 Zones critiques pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['238']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'LOTO' IN p.certifications_sst OR NOT 'Électricité' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'LOTO, Électricité' AS certifications_recommandees
LIMIT 30;
...
// ============================================================================
// 5. 🦴 RISQUES ERGONOMIQUE - 15% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 6. 🧪 RISQUES CHIMIQUE - 12% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Code de sécurité construction, Code électrique
// ============================================================================
...
// Vérification certifications requises secteur: ASP Construction, Travail en hauteur, LOTO, Électricité, Soudage, SIMDUT
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['238']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['ASP Construction', 'Travail en hauteur', 'LOTO', 'Électricité', 'Soudage', 'SIMDUT']]) < 2
RETURN p.matricule AS travailleur,
...
WHERE o.sector_scian IN ['238']
  ...
  AND r.categorie IN ['chute', 'electrique', 'mecanique']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'electrique' THEN 1 ELSE 0 END) AS risques_cat2,
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 238 - SPÉCIALISÉS
