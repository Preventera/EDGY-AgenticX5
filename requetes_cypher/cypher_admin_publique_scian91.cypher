...
// ============================================================================
// Secteur: Administration publique (SCIAN 91)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 4,000
...
// 🧠 PSYCHOSOCIAL: 25% des lésions - Priorité ELEVE
// ⚠️ VIOLENCE: 15% des lésions - Priorité ELEVE
// 🪜 CHUTE: 12% des lésions - Priorité MOYEN
// 🚗 ROUTIER: 8% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Premiers soins, Intervention clientèle difficile
// EPI CRITIQUES: chaise ergonomique, alarme personnelle
// ============================================================================
...
// ============================================================================
// 3. 🧠 RISQUES PSYCHOSOCIAL - 25% des lésions (ELEVE)
// ============================================================================
...
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Premiers soins' IN p.certifications_sst OR NOT 'Intervention clientèle difficile' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Premiers soins, Intervention clientèle difficile' AS certifications_recommandees
LIMIT 30;
...
// ============================================================================
// 5. 🪜 RISQUES CHUTE - 12% des lésions (MOYEN)
// ============================================================================

// 5.1 Identifier tous les risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['91']
...
// 5.2 Travailleurs exposés aux risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
...
// 5.3 Zones critiques pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
...
// ============================================================================
// 6. 🚗 RISQUES ROUTIER - 8% des lésions (MOYEN)
// ============================================================================

// 6.1 Identifier tous les risques routier
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'routier' OR toLower(r.description) CONTAINS 'routier')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 6.2 Travailleurs exposés aux risques routier
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'routier' OR toLower(r.description) CONTAINS 'routier')
...
// 6.3 Zones critiques pour risques routier
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'routier' OR toLower(r.description) CONTAINS 'routier')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques routier
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['91']
  AND (r.categorie = 'routier' OR toLower(r.description) CONTAINS 'routier')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Politique santé fonction publique
// ============================================================================
...
// Vérification certifications requises secteur: Premiers soins, Intervention clientèle difficile
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['91']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Premiers soins', 'Intervention clientèle difficile']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Premiers soins, Intervention clientèle difficile' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['91']
  ...
  AND r.categorie IN ['ergonomique', 'psychosocial', 'violence']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 91 - ADMIN PUBLIQUE
