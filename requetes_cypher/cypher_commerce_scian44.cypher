...
// ============================================================================
// Secteur: Commerce de détail (SCIAN 44)
...
// 🪜 CHUTE: 25% des lésions - Priorité ELEVE
// 🔧 MECANIQUE: 15% des lésions - Priorité MOYEN
// ⚠️ VIOLENCE: 12% des lésions - Priorité ELEVE
// 🧠 PSYCHOSOCIAL: 8% des lésions - Priorité MOYEN
// 🔥 THERMIQUE: 5% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: SIMDUT, Manutention, Cariste, Premiers soins
// EPI CRITIQUES: chaussures sécurité, gants manutention, ceinture lombaire
// ============================================================================
...
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
...
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  ...
  AND (p.certifications_sst IS NULL OR NOT 'Manutention' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Manutention' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 3. 🪜 RISQUES CHUTE - 25% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 4. 🔧 RISQUES MECANIQUE - 15% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// 5. ⚠️ RISQUES VIOLENCE - 12% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 5.2 Travailleurs exposés aux risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// 5.3 Zones critiques pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'SIMDUT' IN p.certifications_sst OR NOT 'Manutention' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, Manutention, Cariste' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. 🧠 RISQUES PSYCHOSOCIAL - 8% des lésions (MOYEN)
// ============================================================================
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Normes travail
// ============================================================================
...
// Vérification certifications requises secteur: SIMDUT, Manutention, Cariste, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'Manutention', 'Cariste', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, Manutention, Cariste' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['44', '441', '442', '443', '444', '445', '44-45']
  ...
  AND r.categorie IN ['ergonomique', 'chute', 'mecanique']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 44 - COMMERCE
