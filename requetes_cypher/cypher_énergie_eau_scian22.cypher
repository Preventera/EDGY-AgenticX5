...
// ============================================================================
// Secteur: Services publics (SCIAN 22)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 2,500
...
// 
// ⚡ ELECTRIQUE: 30% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 20% des lésions - Priorité CRITIQUE
// 🔧 MECANIQUE: 18% des lésions - Priorité ELEVE
// 🕳️ ESPACE_CLOS: 12% des lésions - Priorité CRITIQUE
// 🧪 CHIMIQUE: 10% des lésions - Priorité ELEVE
// 🦴 ERGONOMIQUE: 10% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Électricité haute tension, LOTO, Espace clos, Travail en hauteur, SIMDUT
// EPI CRITIQUES: gants isolants, harnais, casque, lunettes arc flash, détecteur gaz
// ============================================================================
...
// ============================================================================
// 2. ⚡ RISQUES ELECTRIQUE - 30% des lésions (CRITIQUE)
// ============================================================================

// 2.1 Identifier tous les risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
...
// 2.2 Travailleurs exposés aux risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
...
// 2.3 Zones critiques pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
...
// 2.4 Gap certifications pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
...
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Travail en hauteur' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Travail en hauteur' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. 🔧 RISQUES MECANIQUE - 18% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
...
// 4.2 Travailleurs exposés aux risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// 4.3 Zones critiques pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// ============================================================================
// 5. 🕳️ RISQUES ESPACE_CLOS - 12% des lésions (CRITIQUE)
// ============================================================================

// 5.1 Identifier tous les risques espace_clos
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'espace_clos' OR toLower(r.description) CONTAINS 'citerne' OR toLower(r.description) CONTAINS 'puits' OR toLower(r.description) CONTAINS 'réservoir' OR toLower(r.description) CONTAINS 'égout' OR toLower(r.description) CONTAINS 'espace clos')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 5.2 Travailleurs exposés aux risques espace_clos
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'espace_clos' OR toLower(r.description) CONTAINS 'citerne' OR toLower(r.description) CONTAINS 'puits' OR toLower(r.description) CONTAINS 'réservoir' OR toLower(r.description) CONTAINS 'égout' OR toLower(r.description) CONTAINS 'espace clos')
...
// 5.3 Zones critiques pour risques espace_clos
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'espace_clos' OR toLower(r.description) CONTAINS 'citerne' OR toLower(r.description) CONTAINS 'puits' OR toLower(r.description) CONTAINS 'réservoir' OR toLower(r.description) CONTAINS 'égout' OR toLower(r.description) CONTAINS 'espace clos')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques espace_clos
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'espace_clos' OR toLower(r.description) CONTAINS 'citerne' OR toLower(r.description) CONTAINS 'puits' OR toLower(r.description) CONTAINS 'réservoir' OR toLower(r.description) CONTAINS 'égout' OR toLower(r.description) CONTAINS 'espace clos')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Espace clos' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Espace clos' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. 🧪 RISQUES CHIMIQUE - 10% des lésions (ELEVE)
// ============================================================================

// 6.1 Identifier tous les risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
...
// 6.2 Travailleurs exposés aux risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
...
// 6.3 Zones critiques pour risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques chimique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['22']
  AND (r.categorie = 'chimique' OR toLower(r.description) CONTAINS 'acide' OR toLower(r.description) CONTAINS 'silice' OR toLower(r.description) CONTAINS 'poussière' OR toLower(r.description) CONTAINS 'amiante' OR toLower(r.description) CONTAINS 'gaz' OR toLower(r.description) CONTAINS 'chimique' OR toLower(r.description) CONTAINS 'vapeur' OR toLower(r.description) CONTAINS 'solvant')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Code électrique, Hydro-Québec
// ============================================================================
...
// Vérification certifications requises secteur: Électricité haute tension, LOTO, Espace clos, Travail en hauteur, SIMDUT
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['22']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Électricité haute tension', 'LOTO', 'Espace clos', 'Travail en hauteur', 'SIMDUT']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Électricité haute tension, LOTO, Espace clos' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['22']
  ...
  AND r.categorie IN ['electrique', 'chute', 'mecanique']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'electrique' THEN 1 ELSE 0 END) AS risques_cat1,
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 22 - ÉNERGIE/EAU
