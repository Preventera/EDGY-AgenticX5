...
// ============================================================================
// Secteur: Travaux de génie civil (SCIAN 237)
// Profil de risques: CNESST Québec
// Lésions annuelles secteur: 8,000
...
// 
// 🔧 MECANIQUE: 30% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 20% des lésions - Priorité CRITIQUE
// ⛏️ ENSEVELISSEMENT: 15% des lésions - Priorité CRITIQUE
// ⚡ ELECTRIQUE: 10% des lésions - Priorité ELEVE
// 🦴 ERGONOMIQUE: 12% des lésions - Priorité ELEVE
// 🧪 CHIMIQUE: 8% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: ASP Construction, Excavation, Espace clos, Signalisation routière, SIMDUT
// EPI CRITIQUES: casque, gilet haute visibilité, bottes, gants, protecteurs auditifs
// ============================================================================
...
// ============================================================================
// 2. 🔧 RISQUES MECANIQUE - 30% des lésions (CRITIQUE)
// ============================================================================

// 2.1 Identifier tous les risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
...
// 2.2 Travailleurs exposés aux risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
...
// 2.3 Zones critiques pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// 2.4 Gap certifications pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// ============================================================================
// 3. 🪜 RISQUES CHUTE - 20% des lésions (CRITIQUE)
// ============================================================================

// 3.1 Identifier tous les risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
...
// 3.2 Travailleurs exposés aux risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
...
// 3.3 Zones critiques pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 3.4 Gap certifications pour risques chute
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'chute' OR toLower(r.description) CONTAINS 'échelle' OR toLower(r.description) CONTAINS 'escalier' OR toLower(r.description) CONTAINS 'hauteur' OR toLower(r.description) CONTAINS 'plateforme' OR toLower(r.description) CONTAINS 'chute' OR toLower(r.description) CONTAINS 'échafaud' OR toLower(r.description) CONTAINS 'toiture' OR toLower(r.description) CONTAINS 'glissade')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'ASP Construction' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'ASP Construction' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 4. ⛏️ RISQUES ENSEVELISSEMENT - 15% des lésions (CRITIQUE)
// ============================================================================

// 4.1 Identifier tous les risques ensevelissement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ensevelissement' OR toLower(r.description) CONTAINS 'excavation' OR toLower(r.description) CONTAINS 'tranchée' OR toLower(r.description) CONTAINS 'effondrement' OR toLower(r.description) CONTAINS 'ensevelissement')
RETURN z.name AS zone,
       r.description AS risque,
       r.probabilite AS P,
       r.gravite AS G,
...
// 4.2 Travailleurs exposés aux risques ensevelissement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ensevelissement' OR toLower(r.description) CONTAINS 'excavation' OR toLower(r.description) CONTAINS 'tranchée' OR toLower(r.description) CONTAINS 'effondrement' OR toLower(r.description) CONTAINS 'ensevelissement')
...
// 4.3 Zones critiques pour risques ensevelissement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ensevelissement' OR toLower(r.description) CONTAINS 'excavation' OR toLower(r.description) CONTAINS 'tranchée' OR toLower(r.description) CONTAINS 'effondrement' OR toLower(r.description) CONTAINS 'ensevelissement')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques ensevelissement
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ensevelissement' OR toLower(r.description) CONTAINS 'excavation' OR toLower(r.description) CONTAINS 'tranchée' OR toLower(r.description) CONTAINS 'effondrement' OR toLower(r.description) CONTAINS 'ensevelissement')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'ASP Construction' IN p.certifications_sst OR NOT 'Excavation' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'ASP Construction, Excavation, Espace clos' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 5. ⚡ RISQUES ELECTRIQUE - 10% des lésions (ELEVE)
// ============================================================================

// 5.1 Identifier tous les risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
...
// 5.2 Travailleurs exposés aux risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
...
// 5.3 Zones critiques pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques electrique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'electrique' OR toLower(r.description) CONTAINS 'électrique' OR toLower(r.description) CONTAINS 'haute tension' OR toLower(r.description) CONTAINS 'câble' OR toLower(r.description) CONTAINS 'disjoncteur' OR toLower(r.description) CONTAINS 'tension' OR toLower(r.description) CONTAINS 'arc flash')
...
// ============================================================================
// 6. 🦴 RISQUES ERGONOMIQUE - 12% des lésions (ELEVE)
// ============================================================================

// 6.1 Identifier tous les risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
...
// 6.2 Travailleurs exposés aux risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// 6.3 Zones critiques pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques ergonomique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['237']
  AND (r.categorie = 'ergonomique' OR toLower(r.description) CONTAINS 'lever' OR toLower(r.description) CONTAINS 'tirer' OR toLower(r.description) CONTAINS 'manutention' OR toLower(r.description) CONTAINS 'charge' OR toLower(r.description) CONTAINS 'TMS' OR toLower(r.description) CONTAINS 'répétitif' OR toLower(r.description) CONTAINS 'lombaire' OR toLower(r.description) CONTAINS 'posture')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, Code de sécurité construction, MTQ
// ============================================================================
...
// Vérification certifications requises secteur: ASP Construction, Excavation, Espace clos, Signalisation routière, SIMDUT
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['237']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['ASP Construction', 'Excavation', 'Espace clos', 'Signalisation routière', 'SIMDUT']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'ASP Construction, Excavation, Espace clos' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['237']
  ...
  AND r.categorie IN ['mecanique', 'chute', 'ensevelissement']
...
WITH o,
     ...
     sum(CASE WHEN r.categorie = 'mecanique' THEN 1 ELSE 0 END) AS risques_cat1,
     sum(CASE WHEN r.categorie = 'chute' THEN 1 ELSE 0 END) AS risques_cat2,
     sum(CASE WHEN r.probabilite * r.gravite >= 15 THEN 1 ELSE 0 END) AS risques_TZ
RETURN o.name AS organisation,
       o.nb_employes AS employes,
       nb_zones, nb_risques, nb_equipes, nb_personnes,
       round(score_moyen * 100) / 100 AS score_moyen,
       risques_cat1 AS 'mecanique',
       risques_cat2 AS 'chute',
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 237 - GÉNIE CIVIL
