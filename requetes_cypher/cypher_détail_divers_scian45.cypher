...
// ============================================================================
// Secteur: Commerce de détail divers (SCIAN 45)
...
// 
// 🦴 ERGONOMIQUE: 40% des lésions - Priorité CRITIQUE
// 🪜 CHUTE: 20% des lésions - Priorité ELEVE
// ⚠️ VIOLENCE: 15% des lésions - Priorité ELEVE
// 🔧 MECANIQUE: 10% des lésions - Priorité MOYEN
// 🧠 PSYCHOSOCIAL: 10% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: SIMDUT, Manutention, Premiers soins
// EPI CRITIQUES: chaussures antidérapantes, gants
// ============================================================================
...
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
...
// ============================================================================
// 2. 🦴 RISQUES ERGONOMIQUE - 40% des lésions (CRITIQUE)
// ============================================================================
...
// ============================================================================
// 3. 🪜 RISQUES CHUTE - 20% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 4. ⚠️ RISQUES VIOLENCE - 15% des lésions (ELEVE)
// ============================================================================

// 4.1 Identifier tous les risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
...
// 4.2 Travailleurs exposés aux risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
// 4.3 Zones critiques pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 4.4 Gap certifications pour risques violence
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'violence' OR toLower(r.description) CONTAINS 'menace' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'agression' OR toLower(r.description) CONTAINS 'client difficile')
...
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'SIMDUT, Manutention, Premiers soins' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 5. 🔧 RISQUES MECANIQUE - 10% des lésions (MOYEN)
// ============================================================================

// 5.1 Identifier tous les risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
...
// 5.2 Travailleurs exposés aux risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// 5.3 Zones critiques pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 5.4 Gap certifications pour risques mecanique
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  AND (r.categorie = 'mecanique' OR toLower(r.description) CONTAINS 'coupure' OR toLower(r.description) CONTAINS 'coincement' OR toLower(r.description) CONTAINS 'chariot' OR toLower(r.description) CONTAINS 'machine' OR toLower(r.description) CONTAINS 'engrenage' OR toLower(r.description) CONTAINS 'presse' OR toLower(r.description) CONTAINS 'convoyeur' OR toLower(r.description) CONTAINS 'écrasement')
...
// Vérification certifications requises secteur: SIMDUT, Manutention, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['SIMDUT', 'Manutention', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'SIMDUT, Manutention, Premiers soins' AS certifications_prioritaires
LIMIT 50;
...
WHERE o.sector_scian IN ['45', '451', '452', '453', '454', '44-45']
  ...
  AND r.categorie IN ['ergonomique', 'chute', 'violence']
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 45 - DÉTAIL DIVERS
