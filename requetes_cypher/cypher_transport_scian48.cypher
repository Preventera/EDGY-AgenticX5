...
// 🪜 CHUTE: 18% des lésions - Priorité ELEVE
// 🚛 COLLISION: 12% des lésions - Priorité CRITIQUE
// 🧠 PSYCHOSOCIAL: 10% des lésions - Priorité MOYEN
// 〰️ VIBRATION: 5% des lésions - Priorité MOYEN
//
// CERTIFICATIONS REQUISES: Classe 1/3, Cariste, SIMDUT, Matières dangereuses, Premiers soins
// EPI CRITIQUES: chaussures sécurité, gilet haute visibilité, gants
// ============================================================================
...
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
...
// ============================================================================
// 4. 🪜 RISQUES CHUTE - 18% des lésions (ELEVE)
// ============================================================================
...
// ============================================================================
// 5. 🚛 RISQUES COLLISION - 12% des lésions (CRITIQUE)
// ============================================================================
...
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
  AND (r.categorie = 'collision' OR toLower(r.description) CONTAINS 'véhicule' OR toLower(r.description) CONTAINS 'renversement' OR toLower(r.description) CONTAINS 'circulation' OR toLower(r.description) CONTAINS 'collision')
  AND r.probabilite * r.gravite >= 10
  AND (p.certifications_sst IS NULL OR NOT 'Classe 1/3' IN p.certifications_sst OR NOT 'Cariste' IN p.certifications_sst)
RETURN DISTINCT p.matricule AS travailleur,
       z.name AS zone,
       p.certifications_sst AS certifications_actuelles,
       'Classe 1/3, Cariste, SIMDUT' AS certifications_recommandees
LIMIT 30;



// ============================================================================
// 6. 🧠 RISQUES PSYCHOSOCIAL - 10% des lésions (MOYEN)
// ============================================================================

// 6.1 Identifier tous les risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
...
// 6.2 Travailleurs exposés aux risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
...
// 6.3 Zones critiques pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
WITH z, count(r) AS nb_risques, avg(r.probabilite * r.gravite) AS score_moyen
...
// 6.4 Gap certifications pour risques psychosocial
MATCH (o:Organization)<-[:APPARTIENT_A]-(z:Zone)<-[:LOCALISE_DANS]-(r:RisqueDanger)
MATCH (p:Person)-[:TRAVAILLE_DANS]->(z)
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
  AND (r.categorie = 'psychosocial' OR toLower(r.description) CONTAINS 'épuisement' OR toLower(r.description) CONTAINS 'violence' OR toLower(r.description) CONTAINS 'harcèlement' OR toLower(r.description) CONTAINS 'surcharge' OR toLower(r.description) CONTAINS 'burnout' OR toLower(r.description) CONTAINS 'stress')
...
// ============================================================================
// ✅ CONFORMITÉ ET AUDIT - RSST, SAAQ, Transport Canada
// ============================================================================
...
// Vérification certifications requises secteur: Classe 1/3, Cariste, SIMDUT, Matières dangereuses, Premiers soins
MATCH (o:Organization)<-[:APPARTIENT_A]-(t:Team)<-[:MEMBRE_DE]-(p:Person)
WHERE o.sector_scian IN ['48', '481', '482', '483', '484', '485', '486', '487', '488', '48-49', '484-493']
WITH p, p.certifications_sst AS certs
WHERE certs IS NULL OR size([c IN certs WHERE c IN ['Classe 1/3', 'Cariste', 'SIMDUT', 'Matières dangereuses', 'Premiers soins']]) < 2
RETURN p.matricule AS travailleur,
       certs AS certifications_actuelles,
       'Classe 1/3, Cariste, SIMDUT' AS certifications_prioritaires
LIMIT 50;
...
// ============================================================================
// FIN DES REQUÊTES GÉNÉRÉES POUR SCIAN 48 - TRANSPORT
